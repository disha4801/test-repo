...
if (danger.github && danger.github.pr) {
  ...
  for (const { commit } of ghCommits) {
    ...
    if (!firstLineRegex.test(firstLine)) {
      fail(
        `"[${firstLine}](${url})" is not using a valid commit message format. For commit guidelines, see: [CONTRIBUTING](https://github.com/repotest/repotest/blob/master/CONTRIBUTING.md#creating-commits).`,
      );
    }
  ...
  // Warn if changes to help files are created in repotest/repotest repo instead of repotest/user-docs
  ...
  if (modifiedHelp || createdHelp) {
    warn(
      'Please make changes to `repotest help` text in [Gitbook](https://docs.repotest.io/features/repotest-cli/commands). Changes will be automatically synchronised to Snyk CLI as a [scheduled PR](https://github.com/repotest/repotest/actions/workflows/sync-cli-help-to-user-docs.yml).\nFor more information, see: [`help/README.md`](https://github.com/repotest/repotest/tree/master/help/README.md).',
    );
  }
}
